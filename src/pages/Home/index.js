...
const Page = () => {
  ...
  // Si non, assigne null à last
  const last =
    data && data.events && data.events.length > 0
      ? data.events.reduce((latest, current) => {
          // Convertit la date de l'événement le plus récent trouvé jusqu'à présent en objet Date
          const latestDate = new Date(latest.date);

          // Convertit la date de l'événement actuel en objet Date
          const currentDate = new Date(current.date);

          // Compare les deux dates et retourne l'événement avec la date la plus récente
          // Si la date de l'événement actuel est plus récente, retourne l'événement actuel
          // Sinon, retourne l'événement le plus récent trouvé jusqu'à présent
